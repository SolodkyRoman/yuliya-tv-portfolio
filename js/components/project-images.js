...
class ProjectImages extends HTMLElement {
    ...
    getProjectImages() {
        return this.project.images.map(({ alt, id, src }) => Array.isArray(src) ?
            `<div class="project-illustration-split">
                ${src.map(image => `<img class="project-illustration split" src="${image}" alt="${alt}">`).join('')}
            </div>`: `<img class="project-illustration" src="${src}" alt="${alt}" id="${id}" />`).join('');
    }
