...
class ProjectDetails extends HTMLElement {
    ...
    getProjectDescription() {
        return this.project.descriptionBlocks.map(text => `<p class="paragraph">${text}</p>`).join('');
    }

    getProjectCredentials() {
        return this.project.credentials.map(({ key, value }) => `<p class="project-row"><b>${key}</b>: ${value}</p>`).join('');
    }
